...
[![GitHub release](https://img.shields.io/github/v/release/kirvahe/restaurant-skill?sort=semver)](https://github.com/kirvahe/restaurant-skill/releases/latest)
[![License](https://img.shields.io/github/license/kirvahe/restaurant-skill)](LICENSE)
...
saved_places_source: none  # or google_maps / apple_maps  # gmaps_* keys optional (see config.yml.example)
```
...
~/.claude/skills/restaurant/
  ...
  references/                  # Modular reference docs loaded conditionally
    visit-protocol.md          #   Peak/end/disconfirm + decimal scoring
    source-confidence.md       #   A/B/C tier model + tourist-bias rule
...
| references/ | Five modular docs: visit protocol, source confidence (A/B/C tiers), hidden gems, occasion taxonomy (7 occasions), onboarding blocks |
| evals/evals.md | Smoke-test prompts for regression testing after refactors |
...
| config.yml.example | Reference config with all keys documented (`gmaps_*` keys optional) |
| install.sh | One-command installer with `--skill-dir`, `--data-dir`, `--uninstall` flags |
| CHANGELOG.md | Version history (v0.9 → v1.2) |
